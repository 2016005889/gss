<template>
    <div>
        ...
        <el-table :data="tableData" style="width: 100%">
            ...
            <el-table-column prop="grant.reason" label="申请原因" width="80"></el-table-column>

            <el-table-column label="操作">
                <template slot-scope="scope">
                    <!-- {{scope.row.grant.gid}} -->
                    <!-- <el-button size="mini" type="primary"  @click="toEditBase(scope.row)" v-if="scope.row.grant.status < 1">通过</el-button>
                    <el-button size="mini" type="success"  @click="toEditBase(scope.row)" v-if="scope.row.grant.status < 1" disabled="true">已审核</el-button>
                     -->
                     <div v-if="scope.row.grant.status < 1">
                        <el-button size="mini" type="primary"  @click="toUpdateStatus(scope.row.grant.gid)">通过</el-button>
                     </div>
                     <div v-else-if="scope.row.grant.status > 0">
...
export default {
    ...
    methods:{
        reloadDate(){
             // 查询所有用户信息
            request.get("http://127.0.0.1:9999/gss/findAllGrant")
            .then(Response =>{
        ...
        toUpdateStatus(gid){
            ...
            type: 'warning'
                ...
                let url = "http://127.0.0.1:9999/gss/updateGrantStatusByGid"
               
               // console.log(token)
                request.get(url,{params:{gid}})
                .then(Response =>{
                     this.$message({
                        type: 'success',
                        message: '审核成功!'
